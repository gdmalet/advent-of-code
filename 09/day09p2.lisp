...
;;;; This solution is nearly identical to p1, except "shortest" is
...
(defparameter *longest-distance* (cons -1 nil)
...
(defun main ()
  ...
  (format t "Longest route: ~D -> ~A~%"
          (car *longest-distance*)
          (cdr *longest-distance*)))
...
(defun start-at (destinations route distance depth)
  ...
  (when (= depth 0)                  ; bottomed out
    ...
    (when (> distance (car *longest-distance*))
      (format t "best: ~D: ~A~%"  distance (nreverse route))
      (setf *longest-distance* (cons distance (nreverse route))))
